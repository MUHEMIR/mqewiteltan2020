<?php
	function perpage($count, $per_page = '30',$href) {
		$output = '';
		...
		if($pages>1) {
			...
			for($i=($_POST["page"]-20); $i<=($_POST["page"]+1); $i++)	{
				if($i<1) continue;
				if($i>$pages) break;
				if($_POST["page"] == $i)
			...
			if(($pages-($_POST["page"]+20))>1) {
				$output = $output . '...';
			}
			if(($pages-($_POST["page"]+20))>0) {
				if($_POST["page"] == $pages)
	...
	function showperpage($sql, $per_page = 30, $href) {
